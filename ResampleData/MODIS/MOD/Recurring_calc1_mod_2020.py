...
for day in range(int(start_day), int(end_day) + 1):
    # 构建MOD03文件夹路径
    mod03_folder_path = f"/home/LVM_date2/data/GridSatall/MODIS/MOD03/{year}/{day:03d}/"
    # 检查MOD03文件夹是否存在
    if os.path.exists(mod03_folder_path):
        ...
        for mod03_file_name in sorted(mod03_files, key=lambda x: int(x.split(".")[2])):
            if mod03_file_name.startswith("MOD03.A"):
                ...
                # 构建MOD06_L2文件夹路径
                mod06_folder_path = f"/home/LVM_date2/data/GridSatall/MODIS/MOD06_L2/{year}/{day:03d}/"
                # 检查MOD06_L2文件夹是否存在
                if os.path.exists(mod06_folder_path):
                    ...
                    if os.path.exists(mod06_file_path):
                        ...
                        # 输出目录和文件名
                        output_dir = "/home/LVM_date2/zhaolx/Gridsat/Resample_data/MODIS/MOD0.07/" + str(year) + "/" + date_info
