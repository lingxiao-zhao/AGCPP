...
for day in range(int(start_day), int(end_day) + 1):
    # 构建MOD03文件夹路径
    mod03_folder_path = f"/home/LVM_date2/data/MODIS/MOD03/{year}/{day:03d}/"
    # 检查MOD03文件夹是否存在
    if os.path.exists(mod03_folder_path):
        ...
        for mod03_file_name in sorted(mod03_files, key=lambda x: int(x.split(".")[2])):
            if mod03_file_name.startswith("MOD03.A"):
                ...
                if os.path.exists(mod03_file_path):
                    ...
                    # 判断latitude是否在指定范围内
                    if np.all((latitude >= -65) & (latitude <= 65)):
                        # 继续运行代码
                        longitude = mod03_file.variables["Longitude"][:]
                        # 将经度小于0的数据数值加上360
                        longitude[longitude < 0] += 360
                        # 判断longitude最大值是否大于52，如果不大于52，直接读取mod03_file_name中的下一个文件
                        if np.max(longitude) <= 52:
                            mod03_file.close()
                            continue
                        # 判断longitude是否有在0-1内的值，如果有，则直接读取mod03_file_name中的下一个文件
                        if np.any((longitude >= 0) & (longitude <= 1)):
                            mod03_file.close()
                            continue
                        # 判断longitude最小值是否小于214，如果不小于214，直接读取mod03_file_name中的下一个文件
                        if np.min(longitude) >= 214:
                            mod03_file.close()
                            continue
                        # 关闭MOD03文件
                        mod03_file.close()
                ...
                # 构建MOD06_L2文件夹路径
                mod06_folder_path = f"/home/LVM_date2/data/MODIS/MOD06_L2/{year}/{day:03d}/"
                # 检查MOD06_L2文件夹是否存在
                if os.path.exists(mod06_folder_path):
                    ...
                    if os.path.exists(mod06_file_path):
                        ...
                        yy_org = data_array[-2, :, :]
                        # 剔除 NaN 值
                        valid_indices = np.isfinite(xx_org) & np.isfinite(yy_org)
                        xx_valid = xx_org[valid_indices]
                        yy_valid = yy_org[valid_indices]
                        # 目标网格的 x 和 y 坐标范围
                        x_min, x_max = np.ceil(np.min(xx_valid)), np.floor(np.max(xx_valid))
                        y_min, y_max = np.floor(np.min(yy_valid)), np.ceil(np.max(yy_valid))
                        x_min = max(x_min, 52)
                        x_max = min(x_max, 214)
                        y_min = max(y_min, -81)
                        y_max = min(y_max, 81)
                        x_tag_1d = np.arange(x_min, x_max + 0.01, 0.01)  #lon经度
                        y_tag_1d = np.arange(y_min, y_max + 0.005, 0.01)  #lat纬度

                        # 创建一个数组来保存所有通道的数据
                        all_channel_data = np.zeros((5, y_tag_1d.shape[0], x_tag_1d.shape[0]))
                        print("当前处理的MOD06_L2文件:", mod06_file_name)
                        ...
                        # 输出目录和文件名
                        output_dir = "/home/LVM_date2/zhaolx/1/Resample_Data_Code/ResampleData/FY4B2MODIS/MOD/" + date_info
                        output_file_name = "ReMOD06_L2_" + date_info + "_" + time_info + ".nc"
