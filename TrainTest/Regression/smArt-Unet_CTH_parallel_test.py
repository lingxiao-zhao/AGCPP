'''
Description: multi-GPU to train CTH regression model
version: 2.0
Explanation: Original data is augmented with horizontal and vertical flips, data augmentation
...
class loadDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_y = dataset['modis'][1, :, :]    # ["phase", "cth", "cer", "cot", "ctt", "lat", "lon"]
...
def train(proc, nprocs, args, path_train, path_validate, imgpara, labelpara):
    ...
    '''ddp'''
    dist.init_process_group(backend='nccl', init_method='tcp://127.0.0.1:14566', world_size=nprocs, rank=proc) 
    ...
    with torch.no_grad():
        for idx, (vx, vy) in enumerate(valdataloaders):
            ...
            if productname == 'cer':
                ...
                outputstretch = outputstretch[(vy <= 150) & (vy > 0)]
                vy = vy[(vy <= 150) & (vy > 0)]                
            elif productname == 'cth':
...
def test(path):
    ...
    np.random.seed(2021)
    inputDatasets = loadDataset(path, imgpara=imgpara, labelpara=labelpara, productname=productName, augment=False)
    dataloaders = DataLoader(inputDatasets, shuffle=False, batch_size=batch_size, num_workers=2, prefetch_factor=2,
                             pin_memory=True, worker_init_fn=worker_init_fn)

    print(f"测试样本数量: {len(inputDatasets)}")
    preImg = np.zeros((len(inputDatasets), 2, 64, 64))  # 用于存储最终数据

    outputValid = []
    yValid = []
    with torch.no_grad():
        for idx, (x, y) in enumerate(dataloaders):
            print(f"进度: {idx} | {len(dataloaders) - 1}")
            x = x.to(default_device, dtype=torch.float32)
            y = y.to(default_device, dtype=torch.float32)

            out = model(x)
            outputstretch = (out[:, 0, :, :]) * (labelpara[0] - labelpara[1]) + labelpara[1]
            outputstretch = outputstretch.detach().cpu().numpy()
            y = (y * (labelpara[0] - labelpara[1]) + labelpara[1]).detach().cpu().numpy()

            # **使用你原来的 RMSE 计算方式**
            output_flat = outputstretch.flatten()
            y_flat = y.flatten()

            if productName == 'cer':
                mask = (y_flat <= 100) & (y_flat > 0)
            elif productName == 'cot':
                mask = (y_flat <= 150) & (y_flat > 0)
            elif productName == 'cth':
                mask = (y_flat <= 18) & (y_flat > 0)
            elif productName == 'ctt':
                mask = (y_flat <= 340) & (y_flat >= 140)
            else:
                mask = np.ones_like(y_flat, dtype=bool)  # 默认不筛选

            output_filtered = output_flat[mask]
            y_filtered = y_flat[mask]
            if len(output_filtered) != 0:
                print(f"预测最大值: {output_filtered.max():.2f}")
                print(f"预测最小值: {output_filtered.min():.2f}")
                print(f"标签最大值: {y_filtered.max():.2f}")
                print(f"标签最小值: {y_filtered.min():.2f}")
                print(f"RMSE为: {np.sqrt(mean_squared_error(y_filtered, output_filtered)):.2f}")
                print(f"MAE为: {mean_absolute_error(y_filtered, output_filtered):.2f}")
                print("\n")

                outputValid.extend(output_filtered)
                yValid.extend(y_filtered)

            # **只修改 `preImg` 存储方式**
            start_idx = idx * batch_size
            end_idx = start_idx + outputstretch.shape[0]
            preImg[start_idx:end_idx, 0, :, :] = outputstretch
            preImg[start_idx:end_idx, 1, :, :] = y

    # **最终 RMSE、MAE、R²、Pearson 相关系数计算**
    outputValid = np.array(outputValid)
    yValid = np.array(yValid)

    rmse = np.sqrt(mean_squared_error(yValid, outputValid))
    mae = mean_absolute_error(yValid, outputValid)
    r2 = r2_score(yValid, outputValid)
    pearson_r, _ = pearsonr(yValid, outputValid)
    mbe = np.mean(outputValid - yValid)
    print(f"最终 RMSE: {rmse:.3f}")
    print(f"最终 MAE: {mae:.3f}")
    print(f"最终 R²: {r2:.3f}")
    print(f"最终 Pearson R: {pearson_r:.3f}")
    print(f"MBE: {mbe:.3f}")
    # **保存 preImg**
    # save_path = f'/home/nvme/zhaolx/Unet_train/regression/{epochs}_{keys[productIndex]}_2022all.npy'
    # np.save(save_path, preImg)
...
if __name__ == '__main__':
    ...
    parse.add_argument("-action", type=str, dest='action', help="train or test", default="test")
    parse.add_argument("-batch_size", type=int, dest='batch_size', default=1024)
    ...
    parse.add_argument("-ckp", type=str, dest='ckp', help="the path of model weight file",
                        default="/home/nvme/zhaolx/Unet_train/regression/modelsave/cth300-Flip/weights_100_cth_ddp.pth")
    parse.add_argument("-pi", type=int, dest='pi', help="choose the product index", default=2)   ###['cer', 'cot', 'cth', 'ctt']
