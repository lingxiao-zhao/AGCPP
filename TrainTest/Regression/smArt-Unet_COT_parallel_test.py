...
class loadDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_y = dataset['modis'][3, :, :]   # ["phase", "cth", "cer", "cot", "ctt", "lat", "lon"]
...
def train(proc, nprocs, args, path_train, path_validate, imgpara, labelpara):
    ...
    '''ddp'''
    dist.init_process_group(backend='nccl', init_method='tcp://127.0.0.1:14349', world_size=nprocs, rank=proc)
...
def validate(model, valdataloaders, labelpara, proc, productname):
    ...
    # meanRmse = np.sqrt(mean_squared_error(valOutput, valY))
    if proc == 0:
...
def test(path):
    default_device = torch.device("cuda:4") if torch.cuda.is_available() else "cpu"
    ...
    # **保存 preImg**
    save_path = f'/home/nvme/zhaolx/Unet_train/regression/savedata/{epochs}_{keys[productIndex]}_2017-22.npy'
...
if __name__ == '__main__':
    ...
    parse.add_argument("-batch_size", type=int, dest='batch_size', default=256)
    ...
    parse.add_argument("-ckp", type=str, dest='ckp', help="the path of model weight file",
                        default="/home/Data_Pool/zhaolx/Para/weights_286_cot_ddp.pth")
    parse.add_argument("-pi", type=int, dest='pi', help="choose the product index", default=1)   ###['cer', 'cot', 'cth', 'ctt']
